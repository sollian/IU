...
public class ImageManager {
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.what == MESSAGE_LOAD) {
                    ...
                    if (factory != null) {
                        try {
                            ...
                            if (onImageLoadListenner != null) {
                                onImageLoadListenner.onImageLoadFinished(
                    ...
                    if (imageData == null) {
                        ...
                        // 加载clipRect的区域的图片块
                        try {
                            imageData = mDecoder.decodeRegion(clipImageRect,
                                decodingOptions);
                            if (imageData != null) {
                                cacheData.images.put(position, imageData);
                                if (onImageLoadListenner != null) {
                                    onImageLoadListenner
                                        .onBlockImageLoadFinished();
                                }
                            }
                        } catch (Exception e) {
                            Log.d("nnnn", position.toString() + " "
                                + clipImageRect.toShortString());
                            e.printStackTrace();
                        }
                    }
                }
    ...
    private Rect madeRect(Bitmap bitmap, int row, int col, int scaleKey,
                          float imageScale) {
    ...
    public List<DrawData> getDrawData(float imageScale, Rect imageRect) {
        ...
        int maxRow = imageHeight / blockSize
            + (imageHeight % blockSize == 0 ? 0 : 1);
        int maxCol = imageWidth / blockSize
            + (imageWidth % blockSize == 0 ? 0 : 1);

        // 该scale下对应的position范围
        int startRow = imageRect.top / blockSize
            + (imageRect.top % blockSize == 0 ? 0 : 1) - 1;
        int endRow = imageRect.bottom / blockSize
            + (imageRect.bottom % blockSize == 0 ? 0 : 1);
        int startCol = imageRect.left / blockSize
            + (imageRect.left % blockSize == 0 ? 0 : 1) - 1;
        int endCol = imageRect.right / blockSize
            + (imageRect.right % blockSize == 0 ? 0 : 1);
        if (startRow < 0) {
            startRow = 0;
        }
        if (startCol < 0) {
        ...
        if (currentCacheData != null && currentCacheData.scale != scale) {
            cacheDatas.add(new CacheData(currentCacheData.scale,
                new HashMap<Position, Bitmap>(currentCacheData.images)));
        ...
        if (currentCacheData == null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (scale == cacheData.scale) {
                    currentCacheData = new CacheData(scale,
                        new ConcurrentHashMap<Position, Bitmap>(
                            cacheData.images));
                    iterator.remove();
                }
            }
            ...
            for (int row = startRow; row <= endRow; row++) {
                for (int col = startCol; col <= endCol; col++) {
                    ...
                    handler.sendMessage(handler.obtainMessage(what,
                        new MessageData(position, scale)));
                }
            }
            /**
             * <pre>
            ...
            for (int row = startRow; row <= endRow; row++) {
                for (int col = startCol; col <= endCol; col++) {
                    ...
                    if (bitmap == null) {
                        ...
                        Rect rect = madeRect(bitmap, row, col, scale,
                            imageScale);
        ...
        Log.d("countTime", "currentScale time :"
            + (SystemClock.uptimeMillis() - startTime));
        startTime = SystemClock.uptimeMillis();
        ...
        if (!needShowPositions.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (scaleKey / scale == 2) {// 这里都是大于scale的.,缩小的图，单位区域大
                    ...
                    Iterator<Entry<Position, Bitmap>> imageiterator = cacheData.images
                        .entrySet().iterator();
                    while (imageiterator.hasNext()) {
                        Entry<Position, Bitmap> entry = imageiterator.next();
                        Position position = entry.getKey();
                        if (!(startRowKey <= position.row
                            && position.row <= endRowKey
                            && startColKey <= position.col && position.col <= endColKey)) {
                            imageiterator.remove();
                        }
                    }

                    Iterator<Entry<Position, Bitmap>> imagesIterator = cacheData.images
                        .entrySet().iterator();
                    while (imagesIterator.hasNext()) {
                        ...
                        Log.d("nnnn", " bitmap.getWidth():" + bitmap.getWidth()
                        ...
                        for (int row = startPositionRow, i = 0; row <= endPositionRow; row++, i++) {
                            ...
                            for (int col = startPositionCol, j = 0; col <= endPositionCol; col++, j++) {
                                ...
                                if (needShowPositions.remove(new Position(row,
                                    col))) {
                                    ...
                                    rect.bottom = rect.top + (bottom - top)
                                        * scaleKey;
                                    drawDatas.add(new DrawData(bitmap,
                    ...
                    int startRowKey = imageRect.top / size
                        + (imageRect.top % size == 0 ? 0 : 1) - 1;
                    int endRowKey = imageRect.bottom / size
                        + (imageRect.bottom % size == 0 ? 0 : 1);
                    int startColKey = imageRect.left / size
                        + (imageRect.left % size == 0 ? 0 : 1) - 1;
                    int endColKey = imageRect.right / size
                        + (imageRect.right % size == 0 ? 0 : 1);

                    Log.d("nnnn", "startRowKey" + startRowKey + " endRowKey:+"
                        + endRowKey + " endColKey:" + endColKey);
                    ...
                    while (imageiterator.hasNext()) {
                        ...
                        if (!(startRowKey <= position.row
                            ...
                            Bitmap bitmap = entry.getValue();
                            tempPosition.set(position.row / 2
                                    + (position.row % 2 == 0 ? 0 : 1),
                                position.col / 2
                                    + (position.col % 2 == 0 ? 0 : 1));
    ...
    private class NearComparator implements Comparator<CacheData> {
        ...
        public int compare(CacheData lhs, CacheData rhs) {
            int dScale = Math.abs(scale - lhs.scale)
                - Math.abs(scale - rhs.scale);
            if (dScale == 0) {
    ...
    public void load(byte[] data) {
        load(new ByteBitmapRegionDecoderFactory(data));
    }
    ...
    private static class InputStreamBitmapRegionDecoderFactory implements
        BitmapRegionDecoderFactory {
    ...
    private static class ByteBitmapRegionDecoderFactory implements
        BitmapRegionDecoderFactory {
        private byte[] data;

        public ByteBitmapRegionDecoderFactory(byte[] data) {
            super();
            this.data = data;
        }
    ...
    private static class PathBitmapRegionDecoderFactory implements
        BitmapRegionDecoderFactory {
        private String path;
