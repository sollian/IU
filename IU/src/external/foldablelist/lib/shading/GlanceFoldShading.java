...
package external.foldablelist.lib.shading;
...
public class GlanceFoldShading implements FoldShading {
    ...
    @Override
    public void onPreDraw(Canvas canvas, Rect bounds, float rotation,
            int gravity) {
        // NO-OP
    }

    @Override
    public void onPostDraw(Canvas canvas, Rect bounds, float rotation,
            int gravity) {
    ...
    private float getShadowIntencity(float rotation, int gravity) {
        ...
        if (gravity == Gravity.TOP) {
            if (rotation > -90 && rotation < 0) { // (-90; 0) - rotation is
                                                  // applied
    ...
    private boolean computeGlance(Rect bounds, float rotation, int gravity) {
        if (gravity == Gravity.BOTTOM) {
            if (rotation > 0 && rotation < 90) { // (0; 90) - rotation is
                                                 // applied
                final float aspect = (float) mGlance.getWidth()
                        / (float) bounds.width();
                ...
                mGlanceTo.set(bounds.left, bounds.top + distance, bounds.right,
                        bounds.top + distance + scaledGlanceHeight);
                ...
                if (!mGlanceFrom.intersect(0, 0, mGlance.getWidth(),
                        mGlance.getHeight())) {
                    // glance is not visible, should not happen due to previouse
                    // check
                    return false;
                }
