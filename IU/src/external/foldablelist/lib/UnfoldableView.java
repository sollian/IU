...
/**
 * View that provides ability to switch between 2 diffrent views (cover view &
 * details view) with fold animation.
 * <p/>
 * It is implemented as subclass of FoldableListLayout with only 2 views to
 * scroll between.
 */
public class UnfoldableView extends FoldableListLayout {
    ...
    private int mDetailsViewParamWidth, mDetailsViewParamHeight,
            mCoverViewParamWidth, mCoverViewParamHeight;
    ...
    private void setDetailsViewInternal(View detailsView) {
        ...
        // setting precise width/height params and switching details view with
        // it's placeholder
    ...
    private void clearDetailsViewInternal() {
        ...
        // restoring original width/height params and adding cover view back to
        // it's place
    ...
    private void setCoverViewInternal(View coverView) {
        ...
        // setting precise width/height params and switching cover view with
        // it's placeholder
        ...
        // moving cover view into special cover view holder (for unfold
        // animation)
        mCoverHolderLayout.setView(coverView, mCoverViewPosition.width(),
                mCoverViewPosition.height());
    }
    ...
    public void unfold(View coverView, View detailsView) {
        if (mCoverView == coverView && mDetailsView == detailsView)
            return; // already in place

        if ((mCoverView != null && mCoverView != coverView)
                || (mDetailsView != null && mDetailsView != detailsView)) {
            // cover or details view is differ - closing details and schedule
            // reopening
    ...
    public void setFoldRotation(float rotation, boolean isFromUser) {
        ...
        if (mCoverView == null || mDetailsView == null)
            return; // nothing we can do here

        rotation = getFoldRotation(); // parent view will correctly keep
                                      // rotation in bounds for us

        // translating from cover's position to details position
        float stage = rotation / 180; // from 0 = only cover view, to 1 - only
                                      // details view
        ...
        if (mListener != null)
            mListener.onFoldProgress(this, stage);
    ...
    protected void onFoldRotationChanged(FoldableItemLayout layout, int position) {
        ...
        float stage = getFoldRotation() / 180; // from 0 = only cover view, to 1
                                               // - only details view
        ...
        if (position == 0) { // cover view
            // scaling cover view from origin size to the size (width) of the
            // details view
            ...
            float dH = mDetailsViewPosition.height() / 2
                    - mCoverViewPosition.height() * detailsW / coverW;
    ...
    private void switchViews(View origin, View replacement,
    ...
    /**
     * Simple adapter that will alternate between cover view holder layout and
     * details layout
     */
    ...
    /**
     * Cover view holder layout. It can contain at most one child which will be
     * positioned in the top|center_horisontal location of bottom half of the
     * view.
     */
    private static class CoverHolderLayout extends FrameLayout {
        ...
        protected void onLayout(boolean changed, int left, int top, int right,
            ...
            // Collecting visible bounds of child view, it will be used to
            // correctly draw shadows and to
            ...
            if (view != null) {
                ...
                if (foldableLayout != null)
                    foldableLayout.setLayoutVisibleBounds(mVisibleBounds);
            } else {
