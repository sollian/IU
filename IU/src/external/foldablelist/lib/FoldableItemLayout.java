...
/**
 * Layout that provides basic funcitionality: splitting view into 2 parts,
 * splitted parts syncronouse rotations and so on
 */
public class FoldableItemLayout extends FrameLayout {
    ...
    protected void onFinishInflate() {
        ...
        mBaseLayout.moveInflatedChildren(this, 3); // skipping mBaseLayout &
                                                   // mTopPart & mBottomPart
                                                   // views
    }
    ...
    public void setFoldRotation(float rotation) {
        ...
        if (mIsAutoScaleEnabled) {
            ...
            if (mWidth > 0) {
                float dW = (float) (mHeight * Math.abs(Math.sin(Math
                        .toRadians(rotation)))) * CAMERA_DISTANCE_MAGIC_FACTOR;
    ...
    private void setInTransformation(boolean isInTransformation) {
        if (mIsInTransformation == isInTransformation)
            return;
    ...
    private static class BaseLayout extends FrameLayout {
        ...
        private void moveInflatedChildren(FoldableItemLayout layout,
                int firstSkippedItems) {
    ...
    /**
     * Splitted part view. It will draw top or bottom part of cached bitmap and
     * overlay shadows. Also it contains main logic for all transformations
     * (fold rotation, scale, "rolling distance").
     */
    private static class PartView extends View {
        ...
        private void calculateBitmapBounds() {
            if (mBitmap == null) {
                ...
                int top = mGravity == Gravity.TOP ? 0 : (int) (h
                        * (1 - mClippingFactor) - 0.5f);
                int bottom = mGravity == Gravity.TOP ? (int) (h
                        * mClippingFactor + 0.5f) : h;
        ...
        private void applyFoldRotation(float rotation) {
            ...
            position %= 360;
            if (position > 180)
                position -= 360; // now poistion within (-180; 180]

            float rotationX = 0;
            boolean isVisible = true;

            if (mGravity == Gravity.TOP) {
                if (position <= -90 || position == 180) { // (-180; -90] ||
                                                          // {180} - will not
                                                          // show
                    isVisible = false;
        ...
        private void applyRollingDistance(float distance, float scale) {
            ...
            // computing clipping for top view (bottom clipping will be 1 -
            // topClipping)
            final int h = getHeight() / 2;
            final float topClipping = h == 0 ? 0.5f : (h - distance) / h / 2;

            mClippingFactor = mGravity == Gravity.TOP ? topClipping
                    : 1f - topClipping;
        ...
        private void applyVisibility() {
            super.setVisibility(mExtrenalVisibility == VISIBLE ? mInternalVisibility
                    : mExtrenalVisibility);
        }
        ...
        public void draw(Canvas canvas) {
            if (mShading != null)
                mShading.onPreDraw(canvas, mBitmapBounds, mLocalFoldRotation,
                        mGravity);
            if (mBitmap != null)
                canvas.drawBitmap(mBitmap, mBitmapBounds, mBitmapBounds,
                        mBitmapPaint);
            if (mShading != null)
                mShading.onPostDraw(canvas, mBitmapBounds, mLocalFoldRotation,
                        mGravity);
        }
