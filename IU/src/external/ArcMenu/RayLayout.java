...
public class RayLayout extends ViewGroup {
    ...
    public RayLayout(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            a = getContext().obtainStyledAttributes(attrs,
                    R.styleable.RayLayout, 0, 0);
            mLeftHolderWidth = Math.max(a.getDimensionPixelSize(
                    R.styleable.RayLayout_leftHolderWidth, 0), 0);
            a.recycle();

        }
    }

    private static int computeChildGap(final float width, final int childCount,
            final int childSize, final int minGap) {
        return Math.max((int) (width / childCount - childSize), minGap);
    }

    private static Rect computeChildFrame(final boolean expanded,
            final int paddingLeft, final int childIndex, final int gap,
            final int size) {
        final int left = expanded ? (paddingLeft + childIndex * (gap + size) + gap)
                : ((paddingLeft - size) / 2);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, MeasureSpec.makeMeasureSpec(
                getSuggestedMinimumHeight(), MeasureSpec.EXACTLY));

        final int count = getChildCount();
        mChildGap = computeChildGap(getMeasuredWidth() - mLeftHolderWidth,
                count, mChildSize, 0);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            Rect frame = computeChildFrame(mExpanded, paddingLeft, i,
                    mChildGap, mChildSize);
    ...
    private void bindChildAnimation(final View child, final int index,
        ...
        final int childCount = getChildCount();
        Rect frame = computeChildFrame(!expanded, mLeftHolderWidth, index,
                mChildGap, mChildSize);
