...
package external.ListviewAnimations.itemmanipulation.contextualundo;
...
import external.ListviewAnimations.BaseAdapterDecorator;
 ...
 * * Call {@link #setDeleteItemCallback(DeleteItemCallback)} to be notified of
...
public class ContextualUndoAdapter extends BaseAdapterDecorator implements
    ...
    /**
     * Create a new ContextualUndoAdapter based on given parameters. Will
     * automatically remove the swiped item after autoDeleteTimeMillis
     * milliseconds.
     * 
     * @param baseAdapter The {@link BaseAdapter} to wrap
     ...
     * @param autoDeleteTimeMillis The time in milliseconds that the adapter
     *            will wait for he user to hit undo before automatically
     *            deleting the item
     */
    public ContextualUndoAdapter(BaseAdapter baseAdapter, int undoLayoutResId,
            int undoActionResId, int autoDeleteTimeMillis) {
        this(baseAdapter, undoLayoutResId, undoActionResId,
                autoDeleteTimeMillis, -1, null);
    }
     ...
     * @param baseAdapter The {@link BaseAdapter} to wrap
     * @param undoLayoutResId The layout resource id to show as undo
     * @param undoActionResId The resource id of the component which undoes the
     *            dismissal
     * @param autoDeleteTime The time in milliseconds that adapter will wait for
     *            user to hit undo before automatically deleting item
     * @param countDownTextViewResId The resource id of the {@link TextView} in
     *            the undoLayoutResId that will show the time left
     * @param countDownFormatter the {@link CountDownFormatter} which provides
     *            text to be shown in the {@link TextView} as specified by
     *            countDownTextViewResId
     */
    public ContextualUndoAdapter(BaseAdapter baseAdapter, int undoLayoutResId,
            int undoActionResId, int autoDeleteTime,
            int countDownTextViewResId, CountDownFormatter countDownFormatter) {
        super(baseAdapter);
    ...
    public final View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (contextualUndoView == null) {
            contextualUndoView = new ContextualUndoView(parent.getContext(),
                    mUndoLayoutId, mCountDownTextViewResId);
        ...
        View contentView = super.getView(position,
                contextualUndoView.getContentView(), contextualUndoView);
        ...
        if (itemId == mCurrentRemovedId) {
            ...
            long millisLeft = mAutoDeleteDelayMillis
                    - (System.currentTimeMillis() - mDismissStartMillis);
            if (mCountDownFormatter != null) {
                mCurrentRemovedView.updateCountDownTimer(mCountDownFormatter
                        .getCountDownString(millisLeft));
            }
        } else {
    ...
    public void setAbsListView(AbsListView listView) {
        ...
        mContextualUndoListViewTouchListener = new ContextualUndoListViewTouchListener(
                listView, this);
        mContextualUndoListViewTouchListener
        ...
        listView.setOnScrollListener(mContextualUndoListViewTouchListener
                .makeScrollListener());
    ...
    private void startAutoDeleteTimer() {
        ...
        if (mCountDownFormatter != null) {
            mCurrentRemovedView.updateCountDownTimer(mCountDownFormatter
                    .getCountDownString(mAutoDeleteDelayMillis));
        }

        mDismissStartMillis = System.currentTimeMillis();
        mHandler.postDelayed(mCountDownRunnable,
                Math.min(1000, mAutoDeleteDelayMillis));
    }
    ...
    private void performRemovalIfNecessary() {
        if (mCurrentRemovedView != null
                && mCurrentRemovedView.getParent() != null) {
            ValueAnimator animator = ValueAnimator.ofInt(
                    mCurrentRemovedView.getHeight(), 1).setDuration(
                    ANIMATION_DURATION);
     ...
     * statuses.
     * 
     * @param outState the {@link Bundle} provided by
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        mCurrentRemovedId = savedInstanceState.getLong(EXTRA_ACTIVE_REMOVED_ID,
                -1);
    }
    ...
    private class CountDownRunnable implements Runnable {
        ...
        @Override
        public void run() {
            long millisRemaining = mAutoDeleteDelayMillis
                    - (System.currentTimeMillis() - mDismissStartMillis);
            if (mCountDownFormatter != null) {
                mCurrentRemovedView.updateCountDownTimer(mCountDownFormatter
