...
import external.ListviewAnimations.ArrayAdapter;
...
public class SwipeDismissAdapter extends BaseAdapterDecorator {
    ...
    public SwipeDismissAdapter(BaseAdapter baseAdapter,
     ...
     */
    protected SwipeDismissListViewTouchListener createListViewTouchListener(
            AbsListView listView) {
        return new SwipeDismissListViewTouchListener(listView, mCallback,
                mOnScroll);
    }
    ...
    public void setAbsListView(AbsListView listView) {
        ...
        if (mDecoratedBaseAdapter instanceof ArrayAdapter<?>) {
            // fix #35 dirty trick !
            // if ArrayAdapter we assume that items manipulation will come from
            // it
            ((ArrayAdapter<?>) mDecoratedBaseAdapter)
                    .propagateNotifyDataSetChanged(this);
        }
        mSwipeDismissListViewTouchListener = createListViewTouchListener(listView);
        mSwipeDismissListViewTouchListener
