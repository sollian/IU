...
package external.ListviewAnimations.swinginadapters;
...
public abstract class AnimationAdapter extends BaseAdapterDecorator {
    ...
    /**
     * Set the starting position for which items should animate. Given position
     * will animate as well. Will also call setShouldAnimate(true).
     * 
    ...
    /**
     * Set the starting position for which items should animate as the first
     * position which isn't currently visible on screen. This call is also valid
     * when the {@link View}s haven't been drawn yet. Will also call
    ...
    private long calculateAnimationDelay(boolean isHeader) {
        ...
        if (numberOfItems + 1 < mLastAnimatedPosition) {
            ...
            long delaySinceStart = (mLastAnimatedPosition
                    - mFirstAnimatedPosition + 1)
                    * getAnimationDelayMillis();
            delay = mAnimationStartMillis + getInitialDelayMillis()
                    + delaySinceStart - System.currentTimeMillis();
            delay -= isHeader && mLastAnimatedPosition > 0 ? getAnimationDelayMillis()
                    : 0;
        }
    ...
    /**
     * Get the delay in milliseconds before the first animation should start.
     * Defaults to {@value #INITIALDELAYMILLIS}.
     */
