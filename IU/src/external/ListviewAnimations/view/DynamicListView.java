...
package external.ListviewAnimations.view;
...
import external.ListviewAnimations.itemmanipulation.SwipeOnTouchListener;

/**
 * The DynamicListView is an extension of {@link ListView} that supports cell
 * dragging and swapping. </p> Make sure your adapter has stable ids, and
 * override {@link ListAdapter#hasStableIds()} to return true.</br> </p> This
 * layout is in charge of positioning the hover cell in the correct location on
 * the screen in response to user touch events. It uses the position of the
 * hover cell to determine when two cells should be swapped. If two cells should
 * be swapped, all the corresponding data set and layout changes are handled
 * here. </p> If no cell is selected, all the touch events are passed down to
 * the ListView and behave normally. If one of the items in the ListView
 * experiences a long press event, the contents of its current visible state are
 * captured as a bitmap and its visibility is set to INVISIBLE. A hover cell is
 * then created and added to this layout as an overlaying BitmapDrawable above
 * the ListView. Once the hover cell is translated some distance to signify an
 * item swap, a data set change accompanied by animation takes place. When the
 * user releases the hover cell, it animates into its corresponding position in
 * the ListView. </p> When the hover cell is either above or below the bounds of
 * the ListView, this ListView also scrolls on its own so as to reveal
 * additional content. </p> See http://youtu.be/_BZIvjMgH-Q
 */
public class DynamicListView extends ListView {
    ...
    private void updateNeighborViewsForID(long itemID) {
        ...
        mAboveItemId = position - 1 >= 0 ? adapter.getItemId(position - 1)
                : INVALID_ROW_ID;
        mBelowItemId = position + 1 < adapter.getCount() ? adapter
                .getItemId(position + 1) : INVALID_ROW_ID;
    }
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mResIdOfDynamicTouchChild != 0) {
                    ...
                    int position = pointToPosition(mDownX, mDownY);
                    int childNum = (position != INVALID_POSITION) ? position
                            - getFirstVisiblePosition() : -1;
                    View itemView = (childNum >= 0) ? getChildAt(childNum) : null;
                    View childView = (itemView != null) ? itemView
                            .findViewById(mResIdOfDynamicTouchChild) : null;
                    if (childView != null) {
    ...
    /**
     * Interface, usually implemented by a
     * {@link external.ListviewAnimations.BaseAdapterDecorator}, that indicates
     * that it can swap the visual position of two list items.
     * 
    ...
    public interface Swappable {
        ...
        /**
         * Swaps the item on the first adapter position with the item on the
         * second adapter position. Be sure to call
         * {@link android.widget.BaseAdapter#notifyDataSetChanged()} if
         * appropriate.
         * 
