...
import external.GifImageViewEx.com.foxykeep.datadroid.requestmanager.RequestManager;
...
public abstract class RequestService extends MultiThreadedIntentService {
    ...
    public interface Operation {
        /**
         * Execute the request and returns a {@link Bundle} containing the data
         * to return.
         * 
         * @param context The context to use for your operation.
         * @param request The request to execute.
         * @return A {@link Bundle} containing the data to return. If no data to
         *         return, null.
         * @throws ConnectionException Thrown when a connection error occurs. It
         *             will be propagated to the {@link RequestManager} as a
         *             {@link RequestManager#ERROR_TYPE_CONNEXION}.
         * @throws DataException Thrown when a problem occurs while managing the
         *             data of the webservice. It will be propagated to the
         *             {@link RequestManager} as a
         *             {@link RequestManager#ERROR_TYPE_DATA}.
         * @throws CustomRequestException Any other exception you may have to
         *             throw. A call to
         *             {@link RequestService#onCustomRequestException(Request, CustomRequestException)}
         *             will be made with the Exception thrown.
         */
    ...
    /**
     * Proxy method for {@link #sendResult(ResultReceiver, Bundle, int)} when
    ...
    /**
     * Proxy method for {@link #sendResult(ResultReceiver, Bundle, int)} when
     * the work is a failure due to the network.
     * 
     * @param receiver The result receiver received inside the {@link Intent}.
     * @param exception The {@link ConnectionException} triggered.
     */
    private void sendConnexionFailure(ResultReceiver receiver,
            ConnectionException exception) {
        Bundle data = new Bundle();
        data.putInt(RequestManager.RECEIVER_EXTRA_ERROR_TYPE,
                RequestManager.ERROR_TYPE_CONNEXION);
    ...
    /**
     * Proxy method for {@link #sendResult(ResultReceiver, Bundle, int)} when
     * the work is a failure due to the data (parsing for example).
     * 
     * @param receiver The result receiver received inside the {@link Intent}.
     */
    private void sendDataFailure(ResultReceiver receiver) {
        Bundle data = new Bundle();
        data.putInt(RequestManager.RECEIVER_EXTRA_ERROR_TYPE,
                RequestManager.ERROR_TYPE_DATA);
        sendResult(receiver, data, ERROR_CODE);
    }

    /**
     * Proxy method for {@link #sendResult(ResultReceiver, Bundle, int)} when
     * the work is a failure due to {@link CustomRequestException} being thrown.
     * 
     * @param receiver The result receiver received inside the {@link Intent}.
    ...
    private void sendCustomFailure(ResultReceiver receiver, Bundle data) {
        ...
        data.putInt(RequestManager.RECEIVER_EXTRA_ERROR_TYPE,
                RequestManager.ERROR_TYPE_CUSTOM);
        sendResult(receiver, data, ERROR_CODE);
    }
    ...
    private void sendResult(ResultReceiver receiver, Bundle data, int code) {
        DataDroidLog.d(LOG_TAG, "sendResult : "
    ...
    /**
     * Call if a {@link CustomRequestException} is thrown by an
     * {@link Operation}. You may return a Bundle containing data to return to
     * the {@link RequestManager}.
     * <p>
     * Default implementation return null. You may want to override this method
     * in your implementation of {@link RequestService} to execute specific
     * action and/or return specific data.
     * 
     * @param request The {@link Request} which execution threw the exception.
     * @param exception The {@link CustomRequestException} thrown.
     * @return A {@link Bundle} containing data to return to the
     *         {@link RequestManager}. Default implementation return null.
     */
