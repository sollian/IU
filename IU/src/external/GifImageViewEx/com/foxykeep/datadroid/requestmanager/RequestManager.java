...
import external.GifImageViewEx.com.foxykeep.datadroid.util.DataDroidLog;
 ...
 * <p>
 * You can check the following page to see a tutorial on how to implement a
 * webservice call using the {@link RequestManager} : <a
 * href="http://www.datadroidlib.com/installation"
 * >http://www.datadroidlib.com/installation</a>.
 * 
 * @author Foxykeep
 */
public abstract class RequestManager {
    ...
    public static interface RequestListener extends EventListener {
         ...
         * 
         * @param request The {@link Request} defining the request.
         * @param statusCode The HTTP status code returned by the server (if the
         *            request succeeded by the HTTP status code was not
         *            {@link HttpStatus#SC_OK}) or -1 if it was a connection
         *            problem
         */
    ...
    protected RequestManager(Context context,
            Class<? extends RequestService> requestService) {
    ...
    /**
     * Add a {@link RequestListener} to this {@link RequestManager} to a
     * specific {@link Request}. Clients may use it in order to be notified when
     * the corresponding request is completed.
     * <p>
     * The listener is automatically removed when the request is completed and
     * they are notified.
     * <p>
     * <b>Warning !! </b> If it's an {@link Activity} or a {@link Fragment} that
     * is used as a listener, it must be detached when {@link Activity#onPause}
     * is called in an {@link Activity}.
     * 
    ...
    public final void addRequestListener(RequestListener listener,
        ...
        if (requestReceiver == null) {
            DataDroidLog.w(TAG,
                    "You tried to add a listener to a non-existing request.");
            return;
        }
    ...
    /**
     * Remove a {@link RequestListener} to this {@link RequestManager} from
     * every {@link Request}s which it is listening to.
     * 
    ...
    /**
     * Remove a {@link RequestListener} to this {@link RequestManager} from a
     * specific {@link Request}.
     * 
     * @param listener The listener to remove.
     * @param request The {@link Request} associated with this listener. If
     *            null, the listener will be removed from every request it is
    ...
    /**
     * Call the given listener <b>synchronously</b> with the memory cached data
     ...
     * {@link RequestListener#onRequestConnectionError(Request, int)} will be
     * called instead
     * 
    ...
    public final void execute(Request request, RequestListener listener) {
        ...
        DataDroidLog.d(TAG,
                "Creating a new request and adding the listener to it.");
    ...
    private final class ListenerHolder {
        ...
        /* package */void onRequestFinished(Request request, int resultCode,
                Bundle resultData) {
