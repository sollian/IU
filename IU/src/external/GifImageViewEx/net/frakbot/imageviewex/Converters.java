...
public abstract class Converters {
     ...
     * @param opts The decoding options to use, or null if you'd like to use
     *            predefined options (scaling will be not active).
     * @param context The Context for getting the Resources.
     * @return The initialized BitmapDrawable.
     */
    public static BitmapDrawable byteArrayToDrawable(byte[] image,
            Options opts, Context context) {
        if (opts == null) {
            Logcat.v(TAG, "byteArrayToDrawable:opts is null, initializing without scaling");
            opts = new Options();
            opts.inScaled = false;
        }
        Bitmap bmp = BitmapFactory
                .decodeByteArray(image, 0, image.length, opts);
    ...
    public static Bitmap byteArrayToBitmap(byte[] image, Options opts) {
        if (opts == null) {
            Logcat.v(TAG, "byteArrayToBitmap:opts is null, initializing without scaling");
            opts = new Options();
            opts.inScaled = false;
        }
    ...
    /**
     * Gets an asset from a provided AssetManager and its name in the directory
     * and returns a byte array representing the object content.
     * 
     * @param assetManager An {@link AssetManager}.
     * @param asset String of the file name.
     * @return byte[] representing the object content.
     */
    public static byte[] assetToByteArray(AssetManager assetManager,
            String asset) {
        byte[] image = null;
        int b;
        InputStream is = null;
        ...
        try {
            ...
            Logcat.v(TAG, "assetToByteArray:Error while reading asset to byte array: " + asset);
            image = null;
        } finally {
            if (is != null) {
                try {
